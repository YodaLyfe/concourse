...
var _ = Describe("Pool", func() {
	...
	Describe("FindResourceCacheVolume", func() {
		Test("finds a resource cache volume among multiple workers", func() {
			...
			err := scenario.WorkerVolume("worker1", "resource-cache-1").InitializeResourceCache(logger, resourceCache, "worker1")
			Expect(err).ToNot(HaveOccurred())

			err = scenario.WorkerVolume("worker2", "resource-cache-2").InitializeResourceCache(logger, resourceCache, "worker2")
			Expect(err).ToNot(HaveOccurred())
