...
var _ = Describe("Garden Worker", func() {
	...
	Test("fetch image from resource cache volume on same worker", func() {
		...
		err := scenario.WorkerVolume("worker1", "locally-cached-volume").InitializeResourceCache(logger, resourceCache1, "worker1")
		Expect(err).ToNot(HaveOccurred())

		resourceCache2 := scenario.FindOrCreateResourceCache("worker2", "container2")
		err = scenario.WorkerVolume("worker2", "remote-volume").InitializeResourceCache(logger, resourceCache2, "worker2")
		Expect(err).ToNot(HaveOccurred())
