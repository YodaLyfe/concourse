...
func (v Volume) InitializeResourceCache(logger lager.Logger, cache db.UsedResourceCache, sourceWorker string) error {
	if err := v.bcVolume.SetPrivileged(false); err != nil {
		logger.Error("failed-to-set-unprivileged", err)
		return err
	}
	if err := v.dbVolume.InitializeResourceCache(cache, sourceWorker); err != nil {
